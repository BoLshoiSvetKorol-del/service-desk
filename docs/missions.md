...
| 15 | Merge Tickets (Мердж заявок) | Full-stack | 🔜 Следующая |
| 16 | Tags & Notes (Теги и заметки) | Full-stack | ⬜ Запланировано |
| 17 | Client Portal (Клиентский портал) | Full-stack | ⬜ Запланировано |
| 18 | Email Channel (Email-канал) | Full-stack | ⬜ Запланировано |
| 19 | VK Bot | Backend | ⬜ Если останется время |
...
## Миссия 17 — Client Portal (Клиентский портал)

**Цель:** Отдельный интерфейс для внешних клиентов — самостоятельная регистрация, создание и отслеживание своих заявок без доступа к внутренним инструментам.

**Что входит:**

Backend:
- Публичный эндпоинт `POST /api/v1/auth/register` — самостоятельная регистрация (роль `user` по умолчанию)
- Ограничения: клиент видит только свои заявки, не видит внутренние комментарии, не имеет доступа к отчётам
- Email-подтверждение регистрации (Celery task)

Frontend (`/portal/*` — отдельный layout):
- Страница регистрации `/portal/register`
- Страница "Мои заявки" — упрощённый список с поиском
- Страница заявки — только публичные комментарии и статус
- Форма создания заявки — упрощённая (тема + описание + приоритет)

**Зависимости:** 02, 05, 11.

---

## Миссия 18 — Email Channel (Email-канал)
...
```
MVP (01–14) → 15 (Merge) → 16 (Tags+Notes) → 17 (Portal) → 18 (Email) → 19 (VK)
```

**Минимальный рабочий контур (demo-ready):** миссии 01–05, 11–12  
**Полный MVP:** миссии 01–14  
**Полная система по SD.docx:** миссии 01–19
